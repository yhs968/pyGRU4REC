...
def generate_batch(df, batch_size, hidden, training=True, time_sort=False):
    '''
    Args:
    ...
    '''
    # initializations
    click_offsets = get_click_offsets(df)
    session_idx_arr = order_session_idx(df, time_sort=time_sort)
...
def get_click_offsets(df):
    '''
    Return the offsets of the beginning clicks of each session IDs,
    where the offset is calculated against the first click of the first session ID.
    '''

    offsets = np.zeros(df['sid'].nunique()+1, dtype=np.int32)
    # group & sort the df by session_key and get the offset values
    offsets[1:] = df.groupby('sid').size().cumsum()

    return offsets


def order_session_idx(df, time_sort=False):
    '''
    ...
    if time_sort:
        ...
        sessions_start_time = df.groupby('sid')['t'].min().values
        # order the session indices by session starting times
        session_idx_arr = np.argsort(sessions_start_time)
    else:
        session_idx_arr = np.arange(df['sid'].nunique())
